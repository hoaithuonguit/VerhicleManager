...
namespace Data.Repositories
{
    public class ProductRepo : IProductRepo
    {
        ...
        public string AddType(string typeOfProduct, string newTypeName)
        {
            try
            {
                using (var dbContext = new XeNangEntities())
                {
                    var checkIfExists = (from t in dbContext.MasterDatas
                                         where t.Value.Equals(newTypeName)
                                         select t).Count();

                    if (checkIfExists > 0)
                        return Constant.MESSAGE_ERROR;

                    var getCount = (from t in dbContext.MasterDatas select t.ID).Count();
                    int ID = getCount + 1;

                    if (typeOfProduct.Equals(Constant.TYPE_VEHICLE))
                        dbContext.MasterDatas.Add(new MasterData()
                        {
                            ID = ID,
                            Keys = "TypeV",
                            Name = "TPV" + ID,
        ...
        public List<string> GetListProductNameLike(string type, string key)
        {
            ...
            {
                var result = (from p in dbContext.SanPhams
                    where (p.Ten.Contains(key)) && (p.Loai.Equals(type))
                    select p.Ten).ToList<string>();
                if (result.Count > 0)
                    return result;
